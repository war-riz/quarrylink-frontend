 ...
 * 🔌 Wired — all data from useSupplierDashboard (real API).
 */

import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import {
  LayoutGrid, Package, Wallet, Star, ShieldCheck,
  AlertCircle, ChevronRight, LogOut, Edit3, BarChart2,
  X, Plus, Loader2,
} from "lucide-react";
...
import { NotificationCenter, NotificationProvider } from "@/components/dashboard/NotificationSystem";
...
function OrderActionModal({
  order, onClose, onAccept, onReject,
}: {
  order: SupplierOrder;
  onClose: () => void;
  onAccept: () => void;
      ...
      >
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-100 rounded-xl">
            ...
            <p className="text-[12px] text-green-700">Payment secured in escrow. Released after delivery confirmation.</p>
          </div>
        </div>
        {order.status === "new" && (
...
function SupplierSidebar({
  activeTab, setActiveTab, newOrderCount,
}: {
  activeTab: SupplierTab;
  setActiveTab: (t: SupplierTab) => void;
  ...
  const nav = [
    ...
    { id: "orders"    as SupplierTab, label: "Orders",     icon: Package, badge: newOrderCount },
  ...
  return (
    <aside className="hidden lg:flex lg:w-[240px] bg-[#121212] flex-col h-full">
      ...
      <div className="mx-4 mb-5 px-3 py-2.5 rounded-xl bg-green-500/10 border border-green-500/20 flex items-center gap-2">
        ...
        <div>
          <p className="text-[11px] font-bold text-green-400" suppressHydrationWarning>{displayName}</p>
        ...
        {nav.map(({ id, label, icon: Icon, badge }) => {
          ...
          return (
            <button key={id} onClick={() => setActiveTab(id)}
              ...
              <Icon className={cn("w-4 h-4 shrink-0", isActive ? "text-[#121212]" : "text-white/40")} />
      ...
      <div className="p-4 border-t border-white/10">
        <div className="flex items-center gap-3 px-2 py-2">
          <div className="w-9 h-9 rounded-full bg-[#ffc107]/20 border-2 border-[#ffc107]/40 flex items-center justify-center shrink-0">
            <span className="font-bold text-[13px] text-[#ffc107]" suppressHydrationWarning>{initials}</span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-bold text-[13px] text-white truncate" suppressHydrationWarning>{displayName}</p>
...
function SupplierOverview({
  setActiveTab, stats, orders, inventory, newOrderCount, displayName,
}: {
  setActiveTab: (t: SupplierTab) => void;
  stats: ReturnType<typeof import("@/hooks/useSupplierDashboard").useSupplierDashboard>["stats"];
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="bg-[#121212] rounded-2xl p-5 flex items-center justify-between">
        <div>
          <p className="text-[11px] font-bold text-[#ffc107] uppercase tracking-wider mb-1">WELCOME BACK ✋</p>
          <p className="font-extrabold text-[20px] text-white" suppressHydrationWarning>{displayName}</p>
          <p className="text-[13px] text-white/50">Verified Supplier</p>
        </div>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total Revenue", value: formatNaira(stats.totalRevenue), icon: Wallet,      color: "#10b981", bg: "#d1fae5" },
          { label: "In Escrow",     value: formatNaira(stats.escrowHeld),   icon: ShieldCheck, color: "#f59e0b", bg: "#fef3c7" },
          { label: "Avg Rating",    value: `${stats.avgRating} ★`,          icon: Star,        color: "#ffc107", bg: "#fef9c3" },
          { label: "Total Orders",  value: stats.totalOrders.toString(),    icon: Package,     color: "#3b82f6", bg: "#dbeafe" },
        ].map((card, i) => {
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="font-bold text-[14px] text-blue-800">
              {newOrderCount} new order{newOrderCount > 1 ? "s" : ""} awaiting your response
            </p>
            <p className="text-[12px] text-blue-600 mt-0.5">Respond within 2 hours to maintain your acceptance rate</p>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm overflow-hidden">
        ...
        {orders.length === 0 ? (
          <p className="text-center text-zinc-400 text-[13px] py-8">No orders yet.</p>
        ) : (
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm overflow-hidden">
        ...
        <div className="p-5 grid grid-cols-2 gap-3">
          {inventory.length === 0 ? (
            <p className="col-span-2 text-center text-zinc-400 text-[13px] py-4">No inventory listed yet.</p>
          ) : inventory.map(item => (
            <div key={item.id} className={cn("p-4 rounded-xl border",
              item.available ? "border-zinc-100 bg-zinc-50" : "border-red-100 bg-red-50")}>
              ...
              <p className="text-[11px] text-zinc-400">
                {item.available ? `${item.reservedTons}t reserved` : "Out of stock"}
              </p>
            </div>
          ))}
        </div>
...
function SupplierOrders({
  orders, onStatusChange, onAccept, onReject,
}: {
  orders: SupplierOrder[];
  ...
  if (orders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3">
        ...
        <p className="font-bold text-[15px] text-zinc-400">No orders yet</p>
        <p className="text-[13px] text-zinc-300">New orders will appear here</p>
      </div>
    );
  }
              ...
              <div className="flex items-start gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {isNew && (
                      <span className="text-[10px] font-bold bg-blue-500 text-white px-2 py-0.5 rounded-full animate-pulse">NEW</span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <span className={cn("text-[11px] font-bold px-2 py-0.5 rounded-full",
                      order.paymentStatus === "in_escrow" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700")}>
                      {order.paymentStatus === "in_escrow" ? "In Escrow" : "Released"}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <SupplierOrderActionsBar order={order} onStatusChange={onStatusChange} />
...
function SupplierInventory({ inventory }: { inventory: InventoryItem[] }) {
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <p className="text-[13px] text-zinc-400">{inventory.length} materials listed</p>
      ...
      {inventory.length === 0 ? (
        <p className="text-center text-zinc-400 text-[13px] py-12">No inventory listed yet.</p>
      ) : inventory.map(item => (
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="bg-[#121212] rounded-2xl p-6 relative overflow-hidden">
        ...
        <p className="font-extrabold text-[34px] text-white z-10 relative">{formatNaira(stats.totalRevenue)}</p>
        <p className="text-[13px] text-white/50 z-10 relative">Across {stats.completedOrders} completed orders</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        {[
          { label: "Released to You", value: formatNaira(released),       color: "#10b981", bg: "#d1fae5" },
          { label: "In Escrow",       value: formatNaira(stats.escrowHeld), color: "#f59e0b", bg: "#fef3c7" },
        ].map(card => (
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm overflow-hidden">
        ...
        {orders.filter(o => o.paymentStatus !== "pending").length === 0 ? (
          <p className="text-center text-zinc-400 text-[13px] py-8">No payment history yet.</p>
        ) : (
          <div className="divide-y divide-zinc-50">
            {orders.filter(o => o.paymentStatus !== "pending").map(order => (
              <div key={order.id} className="flex items-center gap-4 px-5 py-4">
                <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center shrink-0",
                  ...
                  <Wallet className={cn("w-4 h-4", order.paymentStatus === "released" ? "text-green-500" : "text-amber-500")} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-[13px] text-[#121212] truncate">{order.material}</p>
...
export function SupplierDashboardShell() {
  ...
  const {
    orders, inventory, stats, loading, error,
    newOrderCount, handleStatusChange, acceptOrder, rejectOrder,
  ...
  <div className="flex items-center">
    ...
    {activeTab === "orders" && newOrderCount > 0 && (
      <span className="ml-3 text-[11px] font-bold bg-blue-500 text-white px-2.5 py-1 rounded-full animate-pulse">
        {newOrderCount} New
      </span>
    )}
    {loading && (
      <Loader2 className="ml-3 w-4 h-4 text-zinc-300 animate-spin" />
    )}
    {error && (
      <span className="ml-3 text-[11px] text-red-500 font-semibold">{error}</span>
    )}
  </div>
  <div className="flex items-center gap-2">
    <NotificationCenter />
    <div className="w-9 h-9 rounded-full bg-[#ffc107]/20 border-2 border-[#ffc107]/40 flex items-center justify-center">
      <span className="font-bold text-[12px] text-[#ffc107]" suppressHydrationWarning>{initials}</span>
    </div>
  </div>
</header>
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8">
          <AnimatePresence mode="wait">
            <motion.div key={activeTab}
              ...
              {activeTab === "inventory" && <SupplierInventory inventory={inventory} />}
              {activeTab === "earnings"  && <SupplierEarnings orders={orders} stats={stats} />}
            </motion.div>
          </AnimatePresence>
        </main>
      </div>
